		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				int i = JOptionPane
						.showConfirmDialog(
								null,
								"You will lost Unsaved changes, Do You Really Want To Close ?",
	...
	private boolean isValidData() {
		for (int i = 0; i < stockTable.getRowCount(); i++) {
			for (int j = 0; j < stockTable.getColumnCount(); j++) {
				if (String.valueOf(stockTable.getValueAt(i, j)).trim()
						.equals("")
						|| stockTable.getValueAt(i, j) == null) {
					return false;
				}
			}
		}
	...
	public void actionPerformed(ActionEvent arg) {
		if (arg.getActionCommand().equalsIgnoreCase(addMoreRows.getText())) {
			dm.addRow(new Object[] { ++serialNo });
				...
				.equalsIgnoreCase(removeRows.getText())) {
			if (serialNo > 1) {
				...
				try (PreparedStatement ps = con
					...
					for (int i = 0; i < stockTable.getRowCount(); i++) {
						...
						if ((status = ps.executeUpdate()) == 0) {
							Utility.warningPopup("Adding to Database is failed on S.N. : "
									+ (i + 1) + "...Try Again Later");
						}
					}
					if (status != 0) {
