...
public class ManageAccount extends JFrame implements ActionListener {
		...
		this.addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				int i = JOptionPane.showConfirmDialog(null,
						"You'll lose unsaved Details. Still Want To Close ?",
	...
	private void initValues() {
		...
		try (PreparedStatement ps = con
			...
			Object[] tableColumn = new String[] { "Name", "Email", "Mobile No",
					"Role", "Active" };
			int rowCount = 0;
			while (rs.next()) {
				rowCount++;
	...
	private void updateDetails() {
		Connection con = DBUtil.getCon();
		try (PreparedStatement ps = con
				.prepareStatement("UPDATE sign_up SET role = ?, active = ? WHERE email = ?");) {
			...
			for (int i = 0; i < table.getRowCount(); i++) {
				...
				if ((status = ps.executeUpdate()) == 0) {
					Utility.warningPopup("Updation is failed after all Email of: "
							+ table.getValueAt(i, 1) + "...Try Again Later");
					break;
				}
			}
			if (status != 0) {
